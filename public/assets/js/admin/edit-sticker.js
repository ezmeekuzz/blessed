...
$(document).ready(function() {
    ...
    const editStickerForm = $('#editStickerForm');
    const stickerId = $('#stickerId').val();
    ...
    function setImageSource(source) {
        if (source === 'local') {
            ...
            stickerImage.prop('required', false); // Not required since we can keep existing
    ...
    editStickerForm.on('submit', function(e) {
        ...
        // Make AJAX request
        $.ajax({
            url: '/admin/edit-sticker/update/' + stickerId,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function(response) {
                ...
                if (response.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        text: response.message || 'Sticker has been updated successfully.',
                        confirmButtonColor: '#3D204E'
                    }).then((result) => {
                    ...
                    showToast(response.message || 'Failed to update sticker. Please try again.', 'error');
                }
            },
            error: function(xhr) {
                submitBtn.prop('disabled', false);
