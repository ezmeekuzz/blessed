...
class ProductCategoriesController extends SessionController
{
    public function index()
    {
        $data = [
            'title' => 'The Blessed Manifest | Product Categories',
            'activeMenu' => 'productcategories'
        ];

        return view('pages/admin/product-categories', $data);
    }
    ...
    public function insert()
    {
        ...
        $existingCategory = $productCategoriesModel->where('categoryname', $categoryName)->first();
        if ($existingCategory) {
            return $this->response->setJSON([
                'status' => 'error',
        ...
        try {
            $insertId = $productCategoriesModel->insert($data);
    ...
    public function delete($id)
    {
        ...
        $db = db_connect();
        $postCount = $db->table('product_posts')
        ...
        try {
            if ($productCategoriesModel->delete($id)) {
                return $this->response->setJSON([
                    'status' => 'success',
    ...
    public function update($id)
    {
        ...
        $existingCategory = $productCategoriesModel->where('categoryname', $categoryName)
                                                ->where('product_category_id !=', $id)
                                                ->first();
        if ($existingCategory) {
            return $this->response->setJSON([
        ...
        try {
            if ($productCategoriesModel->update($id, $data)) {
                // Get the updated data
                $updatedCategory = $productCategoriesModel->find($id);
                
                return $this->response->setJSON([
                    'status' => 'success',
                    'message' => 'Category updated successfully',
                    'data' => [
                        'product_category_id' => $updatedCategory['product_category_id'],
