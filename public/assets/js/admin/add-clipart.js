...
$(document).ready(function() {
    ...
    const addClipArtForm = $('#addClipArtForm');
    ...
    function setImageSource(source) {
        if (source === 'local') {
            ...
            clipartImage.prop('required', true);
    ...
    function validateForm() {
        ...
        if (selectedSourceType === 'local') {
            const file = clipartImage[0].files[0];
            if (!file) {
                errors.push('Please select an image file to upload');
                clipartImage.addClass('is-invalid');
                isValid = false;
            ...
            if (!url) {
                ...
                // Optional: Check if URL has image extension
                const imageExtensions = /\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i;
                if (!imageExtensions.test(url)) {
                    errors.push('URL does not appear to point to an image file');
                    // This is just a warning, don't invalidate
                }
            }
        } else {
    ...
    addClipArtForm.on('submit', function(e) {
        e.preventDefault();
            ...
            success: function(response) {
                ...
                if (response.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        text: response.message || 'Clip art has been added successfully.',
                        confirmButtonColor: '#3D204E'
                    }).then((result) => {
                    ...
                    showToast(response.message || 'Failed to add clip art. Please try again.', 'error');
                }
            },
            error: function(xhr) {
                submitBtn.prop('disabled', false);
